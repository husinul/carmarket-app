...
public class require extends Syntax {
    ...
    public static boolean importDefinitions(String className, ModuleInfo info, Procedure renamer, Vector forms, ScopeExp defs, Compilation tr) {
        ...
        if ((info.getState() & 1) == 0 && info.getCompilation() == null) {
            if (!info.checkCurrent(manager, System.currentTimeMillis())) {
        ...
        String str = "find";
        Expression dofind = Invoke.makeInvokeStatic(ClassType.make("kawa.standard.require"), r46, new Expression[]{new QuoteExp(tname)});
        ...
        while (fdecl != null) {
            ...
            if (!fdecl.isPrivate()) {
                ...
                if (!isStatic && decl == null) {
                    decl = new Declaration(Symbol.valueOf(tname.replace('.', '$') + "$instance"), type);
                ...
                if (fdecl.field != null) {
                    if (fdecl.field.getName().endsWith("$instance")) {
                        isImportedInstance = true;
                        old = defs.lookup(aname, language, language.getNamespaceOf(fdecl));
                        if (isImportedInstance) {
                            if (old != null || old.getFlag(512) || Declaration.followAliases(old) != Declaration.followAliases(fdecl)) {
                        ...
                        referenceExp = new ReferenceExp(fdecl);
        ...
        if (isRunnable) {
            ...
            if (decl != null) {
                ...
                Expression[] args = new Expression[]{new QuoteExp(type), new QuoteExp("$instance")};
