    ...
    Expression rewriteClauses(Object clauses, syntax_case_work work, Translator tr) {
        ...
        if (clauses == LList.Empty) {
            ...
            if (this.call_error == null) {
                String str = "error";
                this.call_error = new PrimProcedure(ClassType.make("kawa.standard.syntax_case").addMethod(r25, new Type[]{Compilation.javaStringType, Type.objectType}, Type.objectType, 9), language);
            }
            ...
            if (clauses instanceof Pair) {
                ...
                if (clause instanceof Pair) {
                    ...
                    if (tail instanceof Pair) {
                        ...
                        block = new BlockExp();
                        args = new Expression[4];
                        args[0] = new QuoteExp(syntaxPattern);
                        args[1] = new ReferenceExp(work.inputExpression);
                        args[2] = new ReferenceExp(tr.matchArray);
                        args[3] = new QuoteExp(IntNum.zero());
                        Expression applyExp = new ApplyExp(new PrimProcedure(Pattern.matchPatternMethod, language), args);
