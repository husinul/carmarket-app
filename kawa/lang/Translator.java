...
*/
    ...
    public Expression rewrite_pair(Pair p, boolean function) {
        ...
        if (func instanceof ReferenceExp) {
            ...
            if (decl == null) {
                ...
                if (proc instanceof Syntax) {
                    return apply_rewrite((Syntax) proc, p);
                } else if (proc instanceof AutoloadProcedure) {
                    try {
        ...
        if (st instanceof SyntaxForm) {
            ...
            if (st2 instanceof Pair) {
                ...
                try {
                    ...
                    if (obj instanceof Pair) {
                        Pair p = (Pair) obj;
                        if (p.getCar() == LispLanguage.lookup_sym) {
                            if (p.getCdr() instanceof Pair) {
                                p = (Pair) p.getCdr();
                                if (p.getCdr() instanceof Pair) {
                                    Expression part1 = rewrite(p.getCar());
    ...
    public void finishModule(ModuleExp mexp) {
        ...
        while (decl != null) {
            ...
            if (mexp.getFlag(ModuleExp.EXPORT_SPECIFIED) || (this.generateMain && !this.immediate)) {
            ...
            if (moduleStatic) {
                decl.setFlag(2048);
            } else if ((mexp.getFlag(ModuleExp.NONSTATIC_SPECIFIED) && !decl.getFlag(2048)) || Compilation.moduleStatic < 0 || mexp.getFlag(ModuleExp.SUPERTYPE_SPECIFIED)) {
                decl.setFlag(4096);
            }
            decl = decl.nextDecl();
        }
