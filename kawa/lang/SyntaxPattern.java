...
public class SyntaxPattern extends Pattern implements Externalizable {
    ...
    public boolean match(Object obj, Object[] vars, int start_vars) {
        return match(obj, vars, start_vars, MATCH_MISC, null);
    }
    ...
    SyntaxPattern(StringBuffer programbuf, Object pattern, SyntaxForm syntax, Object[] literal_identifiers, Translator tr) {
        ...
        translate(pattern, programbuf, literal_identifiers, MATCH_MISC, literalsbuf, null, '\u0000', tr);
    ...
    public void disassemble() {
        disassemble(OutPort.errDefault(), (Translator) Compilation.getCurrent(), MATCH_MISC, this.program.length());
    }

    public void disassemble(PrintWriter ps, Translator tr) {
        disassemble(ps, tr, MATCH_MISC, this.program.length());
    }
    ...
    private static int insertInt(int offset, StringBuffer sbuf, int val) {
        ...
        return offset + MATCH_WIDE;
    }

    boolean match_car(Pair p, Object[] vars, int start_vars, int pc, SyntaxForm syntax) {
        int pc_start = pc;
        int pc2 = pc + MATCH_WIDE;
        char ch = this.program.charAt(pc);
        int value = ch >> MATCH_ANY;
        while ((ch & MATCH_ANY_CAR) == MATCH_WIDE) {
            int i = value << 13;
            pc = pc2 + MATCH_WIDE;
            ch = this.program.charAt(pc2);
            value = i | (ch >> MATCH_ANY);
            pc2 = pc;
        }
        if ((ch & MATCH_ANY_CAR) != MATCH_ANY_CAR) {
    ...
    public boolean match(Object obj, Object[] vars, int start_vars, int pc, SyntaxForm syntax) {
        ...
        while (true) {
            if (obj instanceof SyntaxForm) {
                ...
                int pc2 = pc + MATCH_WIDE;
                char ch = this.program.charAt(pc);
                value = (value << 13) | (ch >> MATCH_ANY);
                Pair p;
                int i;
                switch (ch & MATCH_ANY_CAR) {
                    case MATCH_MISC /*0*/:
                        if (ch == MATCH_NIL) {
                            boolean z;
                            if (obj == LList.Empty) {
                    ...
                    case MATCH_LREPEAT /*5*/:
                        ...
                        int subvar0 = ch >> MATCH_ANY;
                        while ((ch & MATCH_ANY_CAR) == MATCH_WIDE) {
                            i2 = subvar0 << 13;
                            pc = pc2 + MATCH_WIDE;
                            ch = this.program.charAt(pc2);
                            subvar0 = i2 | (ch >> MATCH_ANY);
                            pc2 = pc;
                        }
                        subvar0 += start_vars;
                        int subvarN = this.program.charAt(pc2) >> MATCH_ANY;
                        pc2 += MATCH_WIDE;
                        while ((ch & MATCH_ANY_CAR) == MATCH_WIDE) {
                            i2 = subvarN << 13;
                            pc = pc2 + MATCH_WIDE;
                            ch = this.program.charAt(pc2);
                            subvarN = i2 | (ch >> MATCH_ANY);
                            pc2 = pc;
                        }
                        pc = pc2 + MATCH_WIDE;
                        ch = this.program.charAt(pc2);
                        ...
                        if (ch == MATCH_NIL) {
                            ...
                            while ((ch & MATCH_ANY_CAR) == MATCH_WIDE) {
                                ...
                                pc = pc2 + MATCH_WIDE;
                                ch = this.program.charAt(pc2);
                                value = i2 | (ch >> MATCH_ANY);
                                pc2 = pc;
                            }
                            if ((value & MATCH_WIDE) != 0) {
                                listRequired = false;
                            }
                            pairsRequired = value >> MATCH_WIDE;
                            pc = pc2;
                        }
                        ...
                        for (j = MATCH_MISC; j < subvarN; j += MATCH_WIDE) {
                            arrays[j] = new Object[repeat_count];
                        }
                        for (i = MATCH_MISC; i < repeat_count; i += MATCH_WIDE) {
                            SyntaxForm obj2;
    ...
    public static Object[] allocVars(int varCount, Object[] outer) {
        ...
        if (outer != null) {
            System.arraycopy(outer, MATCH_MISC, vars, MATCH_MISC, outer.length);
        }
        return vars;
    }
    ...
    public static Object[] getLiteralsList(Object list, SyntaxForm syntax, Translator tr) {
        ...
        Object[] literals = new Object[(count + MATCH_WIDE)];
        for (int i = MATCH_WIDE; i <= count; i += MATCH_WIDE) {
            SyntaxForm list2;
            Object wrapped;
