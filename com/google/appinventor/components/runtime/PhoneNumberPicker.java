...
public class PhoneNumberPicker extends ContactPicker {
    ...
    public void resultReturned(int requestCode, int resultCode, Intent data) {
        if (requestCode == this.requestCode && resultCode == -1) {
            ...
            String desiredPhoneUri = ElementType.MATCH_ANY_LOCALNAME;
    ...
    public void preHoneycombGetContactInfo(Cursor cursor) {
        if (cursor.moveToFirst()) {
            this.contactName = guardCursorGetString(cursor, NAME_INDEX);
            this.phoneNumber = guardCursorGetString(cursor, NUMBER_INDEX);
            this.contactPictureUri = ContentUris.withAppendedId(People.CONTENT_URI, (long) cursor.getInt(PERSON_INDEX)).toString();
            this.emailAddress = getEmailAddress(guardCursorGetString(cursor, EMAIL_INDEX));
        }
    }
    ...
    public void postHoneycombGetContactEmailsAndPhones(Cursor dataCursor) {
        ...
        if (dataCursor.moveToFirst()) {
            ...
            if (this.emailAddressList.isEmpty()) {
                ...
                this.emailAddress = (String) this.emailAddressList.get(NAME_INDEX);
            }
        }
    }
}
