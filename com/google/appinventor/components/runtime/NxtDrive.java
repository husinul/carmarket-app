...
public class NxtDrive extends LegoMindstormsNxtBase {
    ...
    public void beforeDisconnect(BluetoothConnectionBase bluetoothConnection) {
        if (this.stopBeforeDisconnect) {
            for (Integer intValue : this.driveMotorPorts) {
                setOutputState("Disconnect", intValue.intValue(), REGULATION_MODE_IDLE, REGULATION_MODE_MOTOR_SYNC, REGULATION_MODE_IDLE, REGULATION_MODE_IDLE, REGULATION_MODE_IDLE, 0);
            }
        }
    }
    ...
    public void DriveMotors(String motorPortLetters) {
        ...
        for (int i = REGULATION_MODE_IDLE; i < motorPortLetters.length(); i += REGULATION_MODE_MOTOR_SPEED) {
            char ch = motorPortLetters.charAt(i);
            try {
                this.driveMotorPorts.add(Integer.valueOf(convertMotorPortLetterToNumber(ch)));
            } catch (IllegalArgumentException e) {
                Object[] objArr = new Object[REGULATION_MODE_MOTOR_SPEED];
                objArr[REGULATION_MODE_IDLE] = Character.valueOf(ch);
                this.form.dispatchErrorOccurredEvent(this, "DriveMotors", ErrorMessages.ERROR_NXT_INVALID_MOTOR_PORT, objArr);
            }
        }
    }
    ...
    private void move(String functionName, int power, long tachoLimit) {
        if (checkBluetooth(functionName)) {
            for (Integer intValue : this.driveMotorPorts) {
                setOutputState(functionName, intValue.intValue(), power, REGULATION_MODE_MOTOR_SPEED, REGULATION_MODE_MOTOR_SPEED, REGULATION_MODE_IDLE, MOTOR_RUN_STATE_RUNNING, tachoLimit);
            }
        }
    }
    ...
    public void TurnClockwiseIndefinitely(int power) {
        ...
        if (numDriveMotors >= REGULATION_MODE_MOTOR_SYNC) {
            turnIndefinitely("TurnClockwiseIndefinitely", power, REGULATION_MODE_IDLE, numDriveMotors - 1);
        }
    }
    ...
    public void TurnCounterClockwiseIndefinitely(int power) {
        int numDriveMotors = this.driveMotorPorts.size();
        if (numDriveMotors >= REGULATION_MODE_MOTOR_SYNC) {
            turnIndefinitely("TurnCounterClockwiseIndefinitely", power, numDriveMotors - 1, REGULATION_MODE_IDLE);
        }
    }

    private void turnIndefinitely(String functionName, int power, int forwardMotorIndex, int reverseMotorIndex) {
        if (checkBluetooth(functionName)) {
            setOutputState(functionName, ((Integer) this.driveMotorPorts.get(forwardMotorIndex)).intValue(), power, REGULATION_MODE_MOTOR_SPEED, REGULATION_MODE_MOTOR_SPEED, REGULATION_MODE_IDLE, MOTOR_RUN_STATE_RUNNING, 0);
            setOutputState(functionName, ((Integer) this.driveMotorPorts.get(reverseMotorIndex)).intValue(), -power, REGULATION_MODE_MOTOR_SPEED, REGULATION_MODE_MOTOR_SPEED, REGULATION_MODE_IDLE, MOTOR_RUN_STATE_RUNNING, 0);
        }
    }
    ...
    public void Stop() {
        ...
        if (checkBluetooth(functionName)) {
            for (Integer intValue : this.driveMotorPorts) {
                setOutputState(functionName, intValue.intValue(), REGULATION_MODE_IDLE, REGULATION_MODE_MOTOR_SYNC, REGULATION_MODE_IDLE, REGULATION_MODE_IDLE, REGULATION_MODE_IDLE, 0);
            }
        }
    }
}
