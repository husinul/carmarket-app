...
public class Pedometer extends AndroidNonvisibleComponent implements Component, SensorEventListener, Deleteable {
    ...
    public Pedometer(ComponentContainer container) {
        ...
        this.lastValues = new float[WIN_SIZE];
        this.strideLength = STRIDE_LENGTH;
        this.totalDistance = 0.0f;
        this.stepInterval = new long[NUM_INTERVALS];
    ...
    public void SimpleStep(int simpleSteps, float distance) {
        Object[] objArr = new Object[NUM_INTERVALS];
        objArr[0] = Integer.valueOf(simpleSteps);
        objArr[1] = Float.valueOf(distance);
        EventDispatcher.dispatchEvent(this, "SimpleStep", objArr);
    }
    ...
    public void WalkStep(int walkSteps, float distance) {
        Object[] objArr = new Object[NUM_INTERVALS];
        objArr[0] = Integer.valueOf(walkSteps);
        objArr[1] = Float.valueOf(distance);
        EventDispatcher.dispatchEvent(this, "WalkStep", objArr);
    }
    ...
    private boolean isPeak() {
        int mid = (this.winPos + 50) % WIN_SIZE;
        int i = 0;
        while (i < WIN_SIZE) {
    ...
    public void onSensorChanged(SensorEvent event) {
        if (event.sensor.getType() == 1) {
            ...
            int mid = (this.winPos + 50) % WIN_SIZE;
            if (this.startPeaking && isPeak() && this.foundValley) {
                if (this.lastValues[mid] - this.lastValley > PEAK_VALLEY_RANGE) {
                    long timestamp = System.currentTimeMillis();
                    this.stepInterval[this.intervalPos] = timestamp - this.stepTimestamp;
                    this.intervalPos = (this.intervalPos + 1) % NUM_INTERVALS;
                    this.stepTimestamp = timestamp;
                    if (areStepsEquallySpaced()) {
                        if (this.foundNonStep) {
                            this.numStepsWithFilter += NUM_INTERVALS;
            ...
            this.winPos = (this.winPos + 1) % WIN_SIZE;
        }
    }
