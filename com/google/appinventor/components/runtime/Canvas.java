...
public final class Canvas extends AndroidViewComponent implements ComponentContainer {
    ...
    private final class CanvasView extends View {
        ...
        private void setBackground() {
            int i = Canvas.DEFAULT_BACKGROUND_COLOR;
            Drawable setDraw = this.backgroundDrawable;
            if (Canvas.this.backgroundImagePath == ElementType.MATCH_ANY_LOCALNAME || this.backgroundDrawable == null) {
    ...
    class FlingGestureListener extends SimpleOnGestureListener {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            BoundingBox rect = new BoundingBox((double) Math.max(0, ((int) x) - 12), (double) Math.max(0, ((int) y) - 12), (double) Math.min(Canvas.this.Width() + Canvas.DEFAULT_BACKGROUND_COLOR, ((int) x) + 12), (double) Math.min(Canvas.this.Height() + Canvas.DEFAULT_BACKGROUND_COLOR, ((int) y) + 12));
    ...
    class MotionEventParser {
        ...
        private final List<Sprite> draggedSprites;
        ...
        MotionEventParser() {
            this.draggedSprites = new ArrayList();
            this.startX = -1.0f;
            this.startY = -1.0f;
            this.lastX = -1.0f;
            this.lastY = -1.0f;
        ...
        void parse(MotionEvent event) {
            ...
            BoundingBox rect = new BoundingBox((double) Math.max(0, ((int) x) - 12), (double) Math.max(0, ((int) y) - 12), (double) Math.min(width + UNSET, ((int) x) + HALF_FINGER_WIDTH), (double) Math.min(height + UNSET, ((int) y) + HALF_FINGER_WIDTH));
            boolean handled;
            switch (event.getAction()) {
                ...
                case Canvas.MIN_WIDTH_HEIGHT /*1*/:
                    if (this.drag) {
    ...
    public Canvas(ComponentContainer container) {
        ...
        this.paint.setFlags(MIN_WIDTH_HEIGHT);
        this.paint.setStrokeWidth(DEFAULT_LINE_WIDTH);
        PaintColor(DEFAULT_PAINT_COLOR);
        BackgroundColor(DEFAULT_BACKGROUND_COLOR);
        TextAlignment(MIN_WIDTH_HEIGHT);
    ...
    void addSprite(Sprite sprite) {
        for (int i = 0; i < this.sprites.size(); i += MIN_WIDTH_HEIGHT) {
    ...
    public void Width(int width) {
        if (width > 0 || width == -2 || width == DEFAULT_BACKGROUND_COLOR || width <= Component.LENGTH_PERCENT_TAG) {
            super.Width(width);
        } else {
    ...
    public void Height(int height) {
        if (height > 0 || height == -2 || height == DEFAULT_BACKGROUND_COLOR || height <= Component.LENGTH_PERCENT_TAG) {
            super.Height(height);
        } else {
    ...
    private void changePaint(Paint paint, int argb) {
        if (argb == 0) {
            PaintUtil.changePaint(paint, DEFAULT_PAINT_COLOR);
    ...
    public String Save() {
        try {
            ...
            Object[] objArr = new Object[MIN_WIDTH_HEIGHT];
            objArr[0] = e.getMessage();
            this.container.$form().dispatchErrorOccurredEvent(this, "Save", ErrorMessages.ERROR_MEDIA_FILE_ERROR, objArr);
    ...
    public String SaveAs(String fileName) {
        ...
        try {
            ...
            Object[] objArr = new Object[MIN_WIDTH_HEIGHT];
            objArr[0] = e.getMessage();
            this.container.$form().dispatchErrorOccurredEvent(this, "SaveAs", ErrorMessages.ERROR_MEDIA_FILE_ERROR, objArr);
            return ElementType.MATCH_ANY_LOCALNAME;
        } catch (FileException e2) {
    ...
    private String saveFile(File file, CompressFormat format, String method) {
        ...
        try {
            ...
            objArr = new Object[MIN_WIDTH_HEIGHT];
            objArr[0] = file.getAbsolutePath();
            $form.dispatchErrorOccurredEvent(this, method, ErrorMessages.ERROR_MEDIA_CANNOT_OPEN, objArr);
        } catch (IOException e2) {
            $form = this.container.$form();
            objArr = new Object[MIN_WIDTH_HEIGHT];
            objArr[0] = e2.getMessage();
            $form.dispatchErrorOccurredEvent(this, method, ErrorMessages.ERROR_MEDIA_FILE_ERROR, objArr);
        } catch (Throwable th) {
            fos.close();
        }
    }
