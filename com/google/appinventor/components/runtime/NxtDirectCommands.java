...
public class NxtDirectCommands extends LegoMindstormsNxtBase {
    ...
    public void StopProgram() {
        ...
        if (checkBluetooth(functionName)) {
            sendCommand(functionName, new byte[]{Opcode.UI_FLUSH, (byte) 1});
        }
    }
    ...
    public int GetBatteryLevel() {
        ...
        byte[] command = new byte[]{(byte) 0, ConstantPool.INTERFACE_METHODREF};
        byte[] returnPackage = sendCommandAndReceiveReturnPackage(functionName, command);
        if (!evaluateStatus(functionName, returnPackage, command[1])) {
            return 0;
        }
    ...
    public void StopSoundPlayback() {
        ...
        if (checkBluetooth(functionName)) {
            sendCommand(functionName, new byte[]{Opcode.UI_FLUSH, ConstantPool.NAME_AND_TYPE});
        }
    }
    ...
    public long KeepAlive() {
        ...
        byte[] command = new byte[]{(byte) 0, UIWriteSubcode.ADDRESS};
        byte[] returnPackage = sendCommandAndReceiveReturnPackage(functionName, command);
        if (!evaluateStatus(functionName, returnPackage, command[1])) {
            return 0;
        }
    ...
    public String GetCurrentProgramName() {
        ...
        byte[] command = new byte[]{(byte) 0, UIReadSubcode.GET_WARNING};
        byte[] returnPackage = sendCommandAndReceiveReturnPackage(functionName, command);
    ...
    public String MessageRead(int mailbox) {
        ...
        command[0] = (byte) 0;
        command[1] = UIDrawSubcode.FILLWINDOW;
    ...
    public void DownloadFile(String source, String destination) {
        ...
        if (source.length() == 0) {
            ...
            try {
                ...
                try {
                    InputStream in = new BufferedInputStream(new FileInputStream(tempFile), LambdaExp.SEQUENCE_RESULT);
                    Integer handle;
                    try {
                        byte[] buffer;
                        long sentLength;
    ...
    private Integer openWrite(String functionName, String fileName, long fileSize) {
        ...
        command[0] = (byte) 1;
        command[1] = SystemCommandType.SYSTEM_COMMAND_NO_REPLY;
    ...
    public void SetBrickName(String name) {
        ...
        if (checkBluetooth(functionName)) {
            ...
            command[0] = (byte) 1;
            command[1] = SystemCommand.CLOSE_FILEHANDLE;
    ...
    public String GetBrickName() {
        ...
        byte[] command = new byte[]{(byte) 1, SystemCommand.CREATE_DIR};
        byte[] returnPackage = sendCommandAndReceiveReturnPackage(functionName, command);
        if (evaluateStatus(functionName, returnPackage, command[1])) {
            return getStringValueFromBytes(returnPackage, 3);
        }
