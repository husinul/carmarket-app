...
public class XQResolveNames extends ResolveNames {
    ...
    static {
        handleExtensionDecl = makeBuiltin("(extension)", (int) HANDLE_EXTENSION_BUILTIN);
        castAsDecl = makeBuiltin("(cast as)", (int) CAST_AS_BUILTIN);
        castableAsDecl = makeBuiltin("(castable as)", (int) CASTABLE_AS_BUILTIN);
        lastDecl = makeBuiltin("last", (int) LAST_BUILTIN);
        xsQNameDecl = makeBuiltin(Symbol.make(XQuery.SCHEMA_NAMESPACE, "QName"), (int) XS_QNAME_BUILTIN);
        xsQNameIgnoreDefaultDecl = makeBuiltin(Symbol.make(XQuery.SCHEMA_NAMESPACE, "(QName-ignore-default)"), (int) XS_QNAME_IGNORE_DEFAULT_BUILTIN);
        staticBaseUriDecl = makeBuiltin("static-base-uri", (int) STATIC_BASE_URI_BUILTIN);
        resolvePrefixDecl = makeBuiltin(Symbol.make(XQuery.SCHEMA_NAMESPACE, "(resolve-prefix)"), (int) RESOLVE_PREFIX_BUILTIN);
    }
    ...
    public XQResolveNames(Compilation comp) {
        ...
        pushBuiltin("document", DOC_BUILTIN);
        pushBuiltin("doc-available", DOC_AVAILABLE_BUILTIN);
        pushBuiltin("index-of", INDEX_OF_BUILTIN);
        pushBuiltin(PropertyTypeConstants.PROPERTY_TYPE_STRING, STRING_BUILTIN);
    ...
    protected Expression visitReferenceExp(ReferenceExp exp, ApplyExp call) {
        ...
        if (decl == null) {
            ...
            if (symbol instanceof Symbol) {
                Symbol sym = symbol;
                if (ElementType.MATCH_ANY_LOCALNAME.equals(sym.getNamespaceURI())) {
                    String mname;
            ...
            if (symbol instanceof Symbol) {
                ...
                if (name.indexOf(58) < 0) {
                    ...
                    if (needFunction) {
                        int i = 0;
                        while (true) {
                            int length = this.functionNamespacePath.length;
                            if (i >= r0) {
                                break;
                            }
                ...
                if (decl == null) {
                    ...
                    if (sym2 != null) {
                        ...
                        if (decl == null && (needFunction || needType)) {
                            ...
                            } else if (needType && uri == ElementType.MATCH_ANY_LOCALNAME && !getCompilation().isPedantic()) {
                    ...
                    switch (code) {
                        ...
                        case XS_QNAME_BUILTIN /*-35*/:
                            ...
                            if (code == XS_QNAME_IGNORE_DEFAULT_BUILTIN) {
                                constructorNamespaces = new NamespaceBinding(null, ElementType.MATCH_ANY_LOCALNAME, constructorNamespaces);
                            }
                            ...
                            xargs = new Expression[3];
                            xargs[0] = args[0];
                            xargs[1] = new QuoteExp(constructorNamespaces);
                            xargs[2] = new QuoteExp(this.parser.prologNamespaces);
                            app = new ApplyExp(ClassType.make("gnu.xquery.util.QNameUtils").getDeclaredMethod("resolveQName", 3), xargs);
                            app.setFlag(4);
                            return app;
                        ...
                        case RESOLVE_PREFIX_BUILTIN /*-13*/:
                            ...
                            xargs = new Expression[3];
                            xargs[0] = args[0];
                            xargs[1] = new QuoteExp(this.parser.constructorNamespaces);
                            xargs[2] = new QuoteExp(this.parser.prologNamespaces);
                            app = new ApplyExp(new PrimProcedure(ClassType.make("gnu.xquery.util.QNameUtils").getDeclaredMethod("resolvePrefix", 3)), xargs);
                            app.setFlag(4);
                            return app;
                        ...
                        case HANDLE_EXTENSION_BUILTIN /*-3*/:
                            ...
                            while (true) {
                                if (i < args.length + LAST_BUILTIN) {
                                    ...
                                    if (i < args.length) {
                                        return args[args.length + LAST_BUILTIN];
                                    }
                        ...
                        case POSITION_BUILTIN /*-2*/:
                        case LAST_BUILTIN /*-1*/:
                            sym = code == LAST_BUILTIN ? XQParser.LAST_VARNAME : XQParser.POSITION_VARNAME;
