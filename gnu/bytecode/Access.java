...
public class Access {
    ...
    public static String toString(int flags, char kind) {
        ...
        if ((flags & DateTime.TIMEZONE_MASK) != 0) {
            buf.append(kind == METHOD_CONTEXT ? " varargs" : " transient");
        }
        if ((flags & LambdaExp.NO_FIELD) != 0) {
            buf.append(" native");
        }
        if ((flags & Declaration.NOT_DEFINING) != 0) {
            buf.append(" interface");
        }
        if ((flags & LambdaExp.SEQUENCE_RESULT) != 0) {
            buf.append(" abstract");
        }
        if ((flags & LambdaExp.OVERLOADABLE_FIELD) != 0) {
            buf.append(" strict");
        }
        if ((flags & ModuleExp.EXPORT_SPECIFIED) != 0) {
            buf.append(" enum");
        }
        if ((flags & LambdaExp.ATTEMPT_INLINE) != 0) {
            buf.append(" synthetic");
        }
        if ((flags & Declaration.TYPE_SPECIFIED) != 0) {
