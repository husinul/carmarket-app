...
public class Variable extends Location implements Enumeration {
    ...
    public Variable() {
        this.flags = SIMPLE_FLAG;
        this.offset = UNASSIGNED;
    }
    ...
    private void setFlag(boolean setting, int flag) {
        if (setting) {
            ...
            this.flags &= flag ^ UNASSIGNED;
        }
    }

    public final boolean isSimple() {
        return (this.flags & SIMPLE_FLAG) != 0;
    }
    ...
    public final boolean isParameter() {
        return (this.flags & PARAMETER_FLAG) != 0;
    }
    ...
    public boolean reserveLocal(int varIndex, CodeAttr code) {
        ...
        if (code.locals.used == null) {
            ...
            Variable[] new_locals = new Variable[((code.locals.used.length * PARAMETER_FLAG) + size)];
            System.arraycopy(code.locals.used, 0, new_locals, 0, code.getMaxLocals());
            code.locals.used = new_locals;
        }
        for (j = 0; j < size; j += SIMPLE_FLAG) {
    ...
    public void allocateLocal(CodeAttr code) {
        if (this.offset == UNASSIGNED) {
            for (int i = 0; !reserveLocal(i, code); i += SIMPLE_FLAG) {
            }
        }
    }
    ...
    public void freeLocal(CodeAttr code) {
        this.flags &= -5;
        int vnum = this.offset + (getType().size > LIVE_FLAG ? PARAMETER_FLAG : SIMPLE_FLAG);
        while (true) {
            vnum += UNASSIGNED;
            if (vnum >= this.offset) {
