...
public class ClassTypeWriter extends PrintWriter {
    ...
    public void print() {
        ...
        if ((this.flags & PRINT_CONSTANT_POOL) != 0) {
            printConstantPool();
        }
        printClassInfo();
        printFields();
    ...
    public void printClassInfo() {
        ...
        print("):");
        println();
        for (int i = 0; i < n_interfaces; i += PRINT_CONSTANT_POOL) {
            print("- Implements: ");
    ...
    public void printMethod(Method method) {
        ...
        print('(');
        for (int i = 0; i < method.arg_types.length; i += PRINT_CONSTANT_POOL) {
            if (i > 0) {
                print(',');
            }
    ...
    final void printConstantTersely(CpoolEntry entry, int expected_tag) {
        if (entry == null) {
            ...
            entry.print(this, PRINT_CONSTANT_POOL);
            print('>');
        } else {
            entry.print(this, 0);
        }
    ...
    final void printContantUtf8AsClass(int type_index) {
        ...
        if (entry == null || entry.getTag() != PRINT_CONSTANT_POOL) {
            printConstantTersely(type_index, (int) PRINT_CONSTANT_POOL);
            return;
        }
    ...
    public final void printQuotedString(String string) {
        ...
        for (int i = 0; i < len; i += PRINT_CONSTANT_POOL) {
            ...
            if (ch == '\"') {
                ...
                if (ch != '\n') {
                    ...
                    while (true) {
                        ...
                        print(Character.forDigit((ch >> (j * PRINT_VERSION)) & PRINT_VERBOSE, 16));
                    }
                } else {
                    print("\\n");
                }
    ...
    public void printConstantPool() {
        ...
        for (int i = PRINT_CONSTANT_POOL; i <= length; i += PRINT_CONSTANT_POOL) {
            CpoolEntry entry = pool[i];
            if (entry != null) {
                print('#');
                print(entry.index);
                print(": ");
                entry.print(this, PRINT_CONSTANT_POOL_INDEXES);
                println();
            }
        }
    }
    ...
    public final void printOptionalIndex(int index) {
        if ((this.flags & PRINT_CONSTANT_POOL_INDEXES) != 0) {
            print('#');
            print(index);
            print('=');
        }
    ...
    public final int printSignature(String sig, int pos) {
        ...
        if (c != '(') {
            print(c);
            return pos + PRINT_CONSTANT_POOL;
        }
        pos += PRINT_CONSTANT_POOL;
        print(c);
        int nargs = 0;
        while (pos < len) {
            c = sig.charAt(pos);
            ...
            int nargs2 = nargs + PRINT_CONSTANT_POOL;
            if (nargs > 0) {
                print(',');
            }
