...
public abstract class MethodProc extends ProcedureN {
    ...
    public Type getParameterType(int index) {
        ...
        if (index < atypes.length && (index < atypes.length + NO_MATCH || maxArgs() >= 0)) {
            return atypes[index];
        }
        if (maxArgs() < 0) {
            Type rtype = atypes[atypes.length + NO_MATCH];
    ...
    public static RuntimeException matchFailAsException(int code, Procedure proc, Object[] args) {
        int arg = (short) code;
        if ((code & Component.COLOR_RED) != NO_MATCH_BAD_TYPE) {
            return new WrongArguments(proc, args.length);
        }
        return new WrongType(proc, arg, arg > 0 ? args[arg + NO_MATCH] : null);
    }
    ...
    public static int mostSpecific(MethodProc[] procs, int length) {
        ...
        return best == null ? NO_MATCH : bestn;
    }
}
