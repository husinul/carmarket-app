...
public class WrongType extends WrappedException {
    ...
    public String getMessage() {
        ...
        if (this.number == ARG_DESCRIPTION) {
            sbuf.append(this.procname);
        } else if (this.number == ARG_CAST || this.number == ARG_VARNAME) {
            sbuf.append("Value");
        } else {
        ...
        if (!(this.procname == null || this.number == ARG_DESCRIPTION)) {
            sbuf.append(this.number == ARG_VARNAME ? " for variable '" : " to '");
            sbuf.append(this.procname);
            sbuf.append("'");
        }
        ...
        if (expectType == null && this.number > 0 && (this.proc instanceof MethodProc)) {
            expectType = ((MethodProc) this.proc).getParameterType(this.number + ARG_UNKNOWN);
        }
