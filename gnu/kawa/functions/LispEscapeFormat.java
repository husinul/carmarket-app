...
class LispEscapeFormat extends ReportFormat {
    ...
    static {
        alwaysTerminate = new LispEscapeFormat(0, ParseFormat.PARAM_UNSPECIFIED);
    }

    public LispEscapeFormat(int param1, int param2) {
        this.param1 = param1;
        this.param2 = param2;
        this.param3 = ParseFormat.PARAM_UNSPECIFIED;
    }
    ...
    static Numeric getParam(int param, Object[] args, int start) {
        ...
        if (param != ParseFormat.PARAM_FROM_LIST) {
    ...
    public int format(Object[] args, int start, Writer dst, FieldPosition fpos) throws IOException {
        ...
        if (this.param1 == ParseFormat.PARAM_UNSPECIFIED) {
            if (start != args.length) {
                do_terminate = false;
            }
        } else if (this.param2 == ParseFormat.PARAM_UNSPECIFIED && this.param1 == 0) {
            do_terminate = true;
        } else {
            Numeric arg1 = getParam(this.param1, args, start);
            if (this.param1 == ParseFormat.PARAM_FROM_LIST) {
                start++;
            }
            if (this.param2 == ParseFormat.PARAM_UNSPECIFIED) {
                do_terminate = arg1.isZero();
            } else {
                Numeric arg2 = getParam(this.param2, args, start);
                if (this.param2 == ParseFormat.PARAM_FROM_LIST) {
                    start++;
                }
                if (this.param3 == ParseFormat.PARAM_UNSPECIFIED) {
                    do_terminate = arg1.equals(arg2);
                } else {
                    Numeric arg3 = getParam(this.param3, args, start);
                    if (this.param3 == ParseFormat.PARAM_FROM_LIST) {
                        start++;
                    }
