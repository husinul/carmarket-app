...
public class LispFormat extends CompoundFormat {
                                ...
                                if (ch == ':') {
                                    ...
                                    switch (ch) {
                                        ...
                                        case '<':
                                            ...
                                            pfmt.seenAt = seenAt;
                                            if (seenColon) {
                                                pfmt.prefix = ElementType.MATCH_ANY_LOCALNAME;
                                                pfmt.suffix = ElementType.MATCH_ANY_LOCALNAME;
                                            } else {
                                                pfmt.prefix = "(";
                                                pfmt.suffix = ")";
                                            }
                                        ...
                                        case 'X':
                                            ...
                                            break;
                                        case 'C':
                                            fmt = LispCharacterFormat.getInstance(numParams <= 0 ? getParam(stack, speci) : ParseFormat.PARAM_FROM_LIST, 1, seenAt, seenColon);
                                            break;
                                        case 'I':
                                        ...
                                        case '[':
                                            ...
                                            if (afmt.param == ParseFormat.PARAM_UNSPECIFIED) {
                                                afmt.param = ParseFormat.PARAM_FROM_LIST;
                                            }
                                            if (seenColon) {
    ...
    public static int getParam(Vector vec, int index) {
        if (index >= vec.size()) {
            return ParseFormat.PARAM_UNSPECIFIED;
        }
        String arg = vec.elementAt(index);
        if (arg == paramFromList) {
            return ParseFormat.PARAM_FROM_LIST;
        }
        if (arg == paramFromCount) {
            return ReportFormat.PARAM_FROM_COUNT;
        }
        if (arg != paramUnspecified) {
            return ReportFormat.getParam(arg, ParseFormat.PARAM_UNSPECIFIED);
        }
        return ParseFormat.PARAM_UNSPECIFIED;
    }
