...
public class LispReader extends Lexer {
    ...
    public final void readNestedComment(char c1, char c2) throws IOException, SyntaxException {
        int commentNesting = SCM_NUMBERS;
        ...
        do {
            ...
            if (c3 == '|') {
                ...
                c3 = read();
                if (c3 == c2) {
                    commentNesting += SCM_NUMBERS;
                }
            }
            if (c < 0) {
                eofError("unexpected end-of-file in " + c1 + c2 + " comment starting here", startLine + SCM_NUMBERS, startColumn - 1);
                return;
            }
    ...
    protected Object readAndHandleToken(int ch, int startPos, ReadTable rtable) throws IOException, SyntaxException {
        ...
        if (!this.seenEscapes) {
            Object value = parseNumber(this.tokenBuffer, startPos, endPos - startPos, '\u0000', 0, SCM_NUMBERS);
        ...
        if (readCase == 'I') {
            ...
            while (i < endPos) {
                ...
                if (ci == TOKEN_ESCAPE_CHAR) {
                    ...
                    lowerCount += SCM_NUMBERS;
                } else if (Character.isUpperCase(ci)) {
                    upperCount += SCM_NUMBERS;
                }
                i += SCM_NUMBERS;
            }
            if (lowerCount == 0) {
        ...
        } else if (rtable.initialColonIsKeyword && packageMarker == startPos && len > SCM_NUMBERS) {
            startPos += SCM_NUMBERS;
            return Keyword.make(new String(this.tokenBuffer, startPos, endPos - startPos).intern());
        } else if (rtable.finalColonIsKeyword && packageMarker == endPos - 1 && (len > SCM_NUMBERS || this.seenEscapes)) {
                ...
                if (kind == 3) {
                    ...
                    switch (kind) {
                        ...
                        case SetExp.DEFINING_FLAG /*2*/:
                            if (ch == srfi1.$Pcprovide$Pcsrfi$Mn1 && entry == ReadTableEntry.brace) {
                                braceNesting += SCM_NUMBERS;
                                break;
                            }
    ...
    private boolean isPotentialNumber(char[] buffer, int start, int end) {
        boolean z = true;
        int sawDigits = 0;
        for (int i = start; i < end; i += SCM_NUMBERS) {
            char ch = buffer[i];
            if (Character.isDigit(ch)) {
                sawDigits += SCM_NUMBERS;
            } else if (ch == '-' || ch == '+') {
                if (i + SCM_NUMBERS == end) {
                    return false;
                }
            } else if (ch == '#') {
                return true;
    ...
    public static Object parseNumber(CharSequence str, int radix) {
        ...
        return parseNumber(buf, 0, str.length(), '\u0000', radix, SCM_NUMBERS);
    }
    ...
    public static Object parseNumber(char[] buffer, int start, int count, char exactness, int radix, int flags) {
        ...
        if ((ch == 'i' || ch == 'I') && pos == end && start == pos - 2 && (flags & SCM_NUMBERS) != 0) {
            ...
            if (pos == end) {
                ...
                if (digits_start < 0) {
                    if (sign_seen && pos + 4 < end && buffer[pos + 3] == '.' && buffer[pos + 4] == '0') {
                        if (buffer[pos] == 'i' && buffer[pos + SCM_NUMBERS] == 'n' && buffer[pos + 2] == 'f') {
                            infnan = 'i';
                        } else if (buffer[pos] == 'n' && buffer[pos + SCM_NUMBERS] == 'a' && buffer[pos + 2] == 'n') {
                            infnan = 'n';
                        }
                    }
                    if (infnan == '\u0000') {
                ...
                if (infnan != '\u0000') {
                    ...
                    if (exp_seen >= 0) {
                        ...
                        if (exp_char != 'e') {
                            ...
                            str = str2.substring(0, prefix) + 'e' + str2.substring(prefix + SCM_NUMBERS);
                        }
                    }
    ...
    protected Pair makePair(Object car, Object cdr, int line, int column) {
        ...
        return PairWithPosition.make(car, cdr, pname, line + SCM_NUMBERS, column + SCM_NUMBERS);
    }
    ...
    public static Object readSpecial(LispReader reader) throws IOException, SyntaxException {
        ...
        if (ch == 47 && reader.getLineNumber() == 0 && reader.getColumnNumber() == 3) {
            ReaderIgnoreRestOfLine.getInstance().read(reader, 35, SCM_NUMBERS);
            return Values.empty;
        }
        int startPos = reader.tokenBufferLength;
        reader.tokenBufferAppend(ch);
