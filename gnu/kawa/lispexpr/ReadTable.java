...
public class ReadTable extends RangeTable {
    ...
    public ReadTable() {
        this.postfixLookupOperator = LispReader.TOKEN_ESCAPE_CHAR;
    ...
    public void initialize() {
        ...
        set(94, entry);
        set(95, entry);
        set(srfi1.$Pcprovide$Pcsrfi$Mn1, ReadTableEntry.brace);
        set(126, entry);
        set(127, entry);
        set(8, entry);
    ...
    public ReadTableEntry lookup(int ch) {
        ...
        if (entry != null || ch < 0 || ch >= ModuleExp.NONSTATIC_SPECIFIED) {
            return entry;
        }
        ...
        if (entry == null && ch >= DateTime.TIMEZONE_MASK) {
            entry = ReadTableEntry.getConstituentInstance();
        }
        if (entry == null) {
