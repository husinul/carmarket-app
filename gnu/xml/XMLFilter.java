...
public class XMLFilter implements DocumentHandler, ContentHandler, SourceLocator, XConsumer, PositionConsumer {
    ...
    public NamespaceBinding findNamespaceBinding(String prefix, String uri, NamespaceBinding oldBindings) {
        ...
        info.local = uri;
        info.uri = uri;
        if (uri == ElementType.MATCH_ANY_LOCALNAME) {
            uri = null;
        }
            ...
            if (i == 0) {
                ...
                if (this.attrLocalName == "id" && this.attrPrefix == "xml") {
                    ...
                    while (i3 < valEnd) {
                        i2 = i3 + SAW_KEYWORD;
                        datum = data[i3];
                        if ((65535 & datum) > TreeList.MAX_CHAR_SHORT || datum == '\t' || datum == '\r' || datum == '\n' || (datum == ' ' && (i2 == valEnd || data[i2] == ' '))) {
                ...
                if (this.currentNamespacePrefix != null) {
                    ...
                    data = this.tlist.data;
                    int uriHash = 0;
                    for (i2 = uriStart; i2 < uriEnd; i2 += SAW_KEYWORD) {
                        datum = data[i2];
                        if ((65535 & datum) > TreeList.MAX_CHAR_SHORT) {
                            sbuf = new StringBuffer();
                    ...
                    this.namespaceBindings = lookupNamespaceBinding(this.currentNamespacePrefix == ElementType.MATCH_ANY_LOCALNAME ? null : this.currentNamespacePrefix, data, uriStart, uriLength, uriHash, this.namespaceBindings).namespaces;
    ...
    public void writeDocumentUri(Object uri) {
        if (this.nesting == SAW_WORD && (this.base instanceof TreeList)) {
    ...
    public void writePosition(AbstractSequence seq, int ipos) {
        if (this.ignoringLevel <= 0) {
            if (this.stringizingLevel > 0 && this.previous == SAW_WORD) {
                if (this.stringizingElementNesting < 0) {
                    write(32);
                }
                this.previous = 0;
    ...
    public XMLFilter(Consumer out) {
        this.copyNamespacesMode = SAW_KEYWORD;
        this.previous = 0;
        this.stringizingElementNesting = -1;
        this.startIndexes = null;
        this.attrCount = -1;
        this.namespacePrefixes = false;
        this.mappingTable = new MappingInfo[DateTime.TIMEZONE_MASK];
    ...
    protected void startElementCommon() {
        ...
        if (this.stringizingLevel == 0) {
            ...
            if (this.previous == SAW_WORD && this.stringizingElementNesting < 0) {
                write(32);
            }
            this.previous = 0;
            if (this.stringizingElementNesting < 0) {
    ...
    public void startElement(Object type) {
        ...
        if (this.stringizingLevel == 0) {
            ...
            if (this.copyNamespacesMode == 0) {
                this.namespaceBindings = NamespaceBinding.predefinedXML;
            } else if (this.copyNamespacesMode == SAW_KEYWORD || this.nesting == SAW_WORD) {
                ...
                for (int i = SAW_WORD; i != this.nesting; i += SAW_WORD) {
                    if (this.workStack[i + SAW_KEYWORD] != null) {
                ...
                if (inherited == null) {
                    this.namespaceBindings = type instanceof XName ? ((XName) type).getNamespaceNodes() : NamespaceBinding.predefinedXML;
                } else if (this.copyNamespacesMode == SAW_WORD) {
    ...
    private boolean startAttributeCommon() {
        if (this.stringizingElementNesting >= 0) {
            this.ignoringLevel += SAW_KEYWORD;
        }
        int i = this.stringizingLevel;
        this.stringizingLevel = i + SAW_KEYWORD;
        if (i > 0) {
            return false;
        }
        if (this.attrCount < 0) {
    ...
    public void startAttribute(Object attrType) {
        ...
        if (attrType instanceof Symbol) {
            ...
            if (uri == "http://www.w3.org/2000/xmlns/" || (uri == ElementType.MATCH_ANY_LOCALNAME && local == "xmlns")) {
                error('e', "arttribute name cannot be 'xmlns' or in xmlns namespace");
            }
        }
        if (this.nesting == SAW_WORD && this.workStack[SAW_KEYWORD] == null) {
    ...
    public void emitStartAttribute(char[] data, int start, int count) {
        ...
        if (startAttributeCommon()) {
            ...
            if (prefix != null) {
                ...
                this.currentNamespacePrefix = ElementType.MATCH_ANY_LOCALNAME;
            }
            if (this.currentNamespacePrefix == null || this.namespacePrefixes) {
                this.tlist.startAttribute(0);
            }
    ...
    public void endElement() {
        ...
        if (this.stringizingLevel == 0) {
            ...
            this.workStack[this.nesting + SAW_KEYWORD] = null;
    ...
    public void emitEntityReference(char[] name, int start, int length) {
        char c0 = name[start];
        int ch = '?';
        if (length == SAW_WORD && name[start + SAW_KEYWORD] == 't') {
            if (c0 == 'l') {
                ch = '<';
            } else if (c0 == 'g') {
                ch = '>';
            ...
            if (c0 == 'a' && name[start + SAW_KEYWORD] == 'm' && name[start + SAW_WORD] == 'p') {
                ch = '&';
            }
        } else if (length == 4) {
            char c1 = name[start + SAW_KEYWORD];
            char c2 = name[start + SAW_WORD];
    ...
    public void emitCharacterReference(int value, char[] name, int start, int length) {
        if (value >= ModuleExp.NONSTATIC_SPECIFIED) {
    ...
    public void processingInstructionFromParser(char[] buffer, int tstart, int tlength, int dstart, int dlength) {
        if (tlength != 3 || inElement() || buffer[tstart] != 'x' || buffer[tstart + SAW_KEYWORD] != 'm' || buffer[tstart + SAW_WORD] != 'l') {
    ...
    MappingInfo lookupTag(char[] data, int start, int length) {
        ...
        int prefixHash = 0;
        int colon = -1;
        for (int i = 0; i < length; i += SAW_KEYWORD) {
    ...
    private void ensureSpaceInWorkStack(int oldSize) {
        if (this.workStack == null) {
            ...
            Object[] tmpn = new Object[(this.workStack.length * SAW_WORD)];
    ...
    private void ensureSpaceInStartIndexes(int oldSize) {
        if (this.startIndexes == null) {
            ...
            int[] tmpn = new int[(this.startIndexes.length * SAW_WORD)];
    ...
    public void startElement(String namespaceURI, String localName, String qName, Attributes atts) {
        ...
        for (int i = 0; i < numAttributes; i += SAW_KEYWORD) {
    ...
    public void startElement(String name, AttributeList atts) {
        ...
        for (int i = 0; i < attrLength; i += SAW_KEYWORD) {
