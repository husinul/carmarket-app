...
public class NamedUnit extends Unit implements Externalizable {
    ...
    public static NamedUnit lookup(String name) {
        name = name.intern();
        for (NamedUnit unit = table[(Integer.MAX_VALUE & name.hashCode()) % table.length]; unit != null; unit = unit.chain) {
            if (unit.name == name) {
                return unit;
            }
        }
    ...
    public static NamedUnit lookup(String name, double scale, Unit base) {
        name = name.intern();
        NamedUnit unit = table[(Integer.MAX_VALUE & name.hashCode()) % table.length];
        while (unit != null) {
